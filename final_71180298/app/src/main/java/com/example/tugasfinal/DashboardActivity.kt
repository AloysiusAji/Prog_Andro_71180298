    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.btnCreate.setOnClickListener(View.OnClickListener {
            if(binding.nomorIdentitas.text.isNotEmpty() && binding.namaPenduduk.text.isNotEmpty()
                ...
                val penduduk = hashMapOf(
                    "Nomor_Identitas" to binding.nomorIdentitas.text.toString(),
                    "Nama_Penduduk" to binding.namaPenduduk.text.toString(),
                    "Usia" to binding.usiaPenduduk.text.toString(),
                ...
                db.collection("Penduduk")
                    ...
                    .addOnSuccessListener { documentReference ->
                        Toast.makeText(applicationContext, "Updated: ${documentReference.id}", Toast.LENGTH_SHORT).show()
                ...
                Toast.makeText(applicationContext, "Form cant empty!", Toast.LENGTH_SHORT).show()
            }
        })
            ...
            db.collection("Penduduk")
                ...
                .addOnSuccessListener { result ->
                    for (document in result) {
                        data.append("Title : " + document.get("Nomor_Identitas") + "\n")
                        data.append("Artist : " + document.get("Nama_Penduduk") + "\n")
                        data.append("Year : " + document.get("Usia") + "\n")
                        data.append("Studio : " + document.get("Alamat") + "\n" + "\n")
                    }
                    displayMessage("List Song", data.toString())
                }
        ...
        binding.btnDelete.setOnClickListener(View.OnClickListener {
            ...
            if(binding.nomorIdentitas.text.isNotEmpty()) {
                val query = db.collection("Penduduk")
                    .whereEqualTo("Nomor_Identitas", binding.nomorIdentitas.text.toString())
                    .get()
            ...
            // Hapus Data Berdasarkan Usia
                ...
                val query = db.collection("Penduduk")
                    .whereEqualTo("Usia", binding.usiaPenduduk.text.toString())
                    .get()
                query.addOnSuccessListener {
                    for(document in it) {
                        db.collection("Penduduk").document(document.id).delete().addOnSuccessListener {
        ...
        binding.btnUpdate.setOnClickListener(View.OnClickListener {
            if(binding.nomorIdentitas.text.isNotEmpty() && binding.namaPenduduk.text.isNotEmpty()
                ...
                // Create a new user with a first and last name
                val penduduk = hashMapOf(
                    "Title" to binding.nomorIdentitas.text.toString(),
                    "Artist" to binding.namaPenduduk.text.toString(),
                    "Year" to binding.usiaPenduduk.text.toString(),
                    "Studio" to binding.alamatPenduduk.text.toString()
                )
